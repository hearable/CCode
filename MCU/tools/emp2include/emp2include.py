...
def process(memory):
	...
	for file in asps:
		...
		# Add the hex representation of the patch Build Number Version
		S += '0x{:04X}'.format(binarray[12] + (binarray[13] << 8))
		S += ', '

		# Add the hex representation of the patch User Build Number
		S += '0x{:04X}'.format(binarray[14] + (binarray[15] << 8))
		S += ', '

		# Add the hex representation of the patch Format Version
		S += '0x{:02X}'.format(binarray[8])
		S += ', '

		# Add the hex representation of the patch Container Type
		S += '0x{:02X}'.format(binarray[9])
		S += ', '

		# Add the hex representation of the patch Container ID
		S += '0x{:02X}'.format(binarray[10])
		S += ', '
	...
	dir = os.path.join(os.getcwd(),'../../third_party/exactle/sw/hci/ambiq/em9304')
	os.chdir(dir)
