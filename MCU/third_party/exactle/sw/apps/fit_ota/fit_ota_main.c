 ...
 *  \file   fit_ota_main.c
 *
 *  \brief  Fitness application modified to support Ambiq Micro's demonstration of AMOTA service.
 *
...
static const AmotasCfg_t fitAmotasCfg =
{
    0
};
...
enum
{
  ...
  FIT_AMOTAS_TX_CCC_IDX,                  /*! AMOTA service, tx characteristic */
  FIT_NUM_CCC_IDX
};
...
static const attsCccSet_t fitCccSet[FIT_NUM_CCC_IDX] =
{
  ...
  {AMOTAS_TX_CH_CCC_HDL,  ATT_CLIENT_CFG_NOTIFY,    DM_SEC_LEVEL_NONE}    /* FIT_AMOTAS_TX_CCC_IDX */
};
...
static void fitProcCccState(fitMsg_t *pMsg)
{
  ...
  /* AMOTAS TX CCC */
  if (pMsg->ccc.idx == FIT_AMOTAS_TX_CCC_IDX)
  {
    if (pMsg->ccc.value == ATT_CLIENT_CFG_NOTIFY)
    {
      // notify enabled
...
void FitHandlerInit(wsfHandlerId_t handlerId)
{
  ...
  amotas_init(handlerId, (AmotasCfg_t *) &fitAmotasCfg);
}
